function [llik,mode,modeV]=llik_fun_IS(x,theta,rand_eta,rand_u,N)     
%debuggin
    %theta(1)=theta_hat_est(1) ;
    %theta(2)=theta_hat_est(2); 
    %theta(3)=theta_hat_est(3) ;
    %x=y;
   % rand_eta=rand_eta;
   % rand_u=rand_u;
...
while difference>0.00001
    ...
    z= g + I - exp(g)./vy.^2;
      Yn = z;
      H = A;    
    [llik_g,modsmooth] = kf_smooth(Yn,H,mT,0,0,sigma2_eta,a0,P0);
...
[~,con_theta_hat] =kf_smooth(y_star,A,phi,0,0,sigma2_eta,0,P0);
...
[~,con_theta_hat_plus] =kf_smooth(y_plus,A,phi,0,0,sigma2_eta,0,P0);
